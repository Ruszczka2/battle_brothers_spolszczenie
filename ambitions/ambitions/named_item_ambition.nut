this.named_item_ambition <- this.inherit("scripts/ambitions/ambition", {
	...
	function create()
	{
		...
		this.m.SuccessText = "[img]gfx/ui/events/event_28.png[/img]Nie było łatwo odnaleźć jeden z tych rzadkich przedmiotów, uznawanych za talizman wojowników, ale %nameditem% jest naprawdę niezwykły. Ludzie niemal promienieją dumą po jego zdobyciu. Przy ognisku niektórzy bracia robią się rozczuleni i łzawią pod wpływem trunków, mizając go jak ulubionego psa.%SPEECH_ON%Czyż nie jest piękny?%SPEECH_OFF%%randombrother% pyta, podziwiając go w blasku ogniska, szybko uciszony przez innego z ludzi.%SPEECH_ON%Obróć go tak, żebym mógł się dobrze przyjrzeć!%SPEECH_OFF%W kolejnych dniach twoi ludzie obnoszą %nameditem% niczym trofealną głowę jakiejś straszliwej bestii. Zwykle w tawernach albo na festynach i w pobliżu innych źródeł miodu pitnego i piwa bracia przywykli pokazywać swoją zdobycz wszędzie, gdzie się pojawicie.";

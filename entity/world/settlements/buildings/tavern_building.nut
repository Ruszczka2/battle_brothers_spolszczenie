this.tavern_building <- this.inherit("scripts/entity/world/settlements/buildings/building", {
	...
	function getRumor( _isPaidFor = false )
	{
		...
		if (this.m.RumorsGiven > 3)
		{
			if (_isPaidFor)
			{
				return "Bywalcy wznoszą kubki na twoją cześć, choć wydaje się, dziś już niczego ciekawego się od nich nie dowiesz.";
			}
			else
			{
				return "Bywalcy rozmawiają o tym i o tamtym.";
			}
		}
		else
		{
			...
			{
				rumor = rumor + "Bywalcy rozmawiają o tym i o tamtym.";
			}
	...
	function getDrinkResult()
	{
		...
		foreach( b in bros )
		{
			...
			if (this.Math.rand(1, 100) <= drunkChance)
			{
				...
				result.Result.push({
					Icon = drunk.getIcon(),
					Text = b.getName() + " jest teraz pijany"
				});
			}
			...
			if ((b.getLastDrinkTime() == 0 || this.Time.getVirtualTimeF() - b.getLastDrinkTime() > this.World.getTime().SecondsPerDay) && this.Math.rand(1, 100) <= 35)
			{
				...
				b.improveMood(this.Const.MoodChange.DrunkAtTavern, "Upił się wraz z kompanią");
				result.Result.push({

this.send_bandit_roamers_action <- this.inherit("scripts/factions/faction_action", {
	...
	function onExecute( _faction )
	{
		...
		local party = this.getFaction().spawnEntity(settlement.getTile(), "Bandyci Myτliwi", false, this.Const.World.Spawn.BanditRoamers, this.Math.min(settlement.getResources(), this.Math.rand(60, 110)));
		party.getSprite("banner").setBrush(settlement.getBanner());
		party.setDescription("Szajka zatwardziałych bandytów polująca na zwierzynę łowną.");

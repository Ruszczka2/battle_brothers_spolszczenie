this.send_caravan_action <- this.inherit("scripts/factions/faction_action", {
	...
	function onExecute( _faction )
	{
		...
		{
			party = _faction.spawnEntity(this.m.Start.getTile(), "Karawana Kupiecka", false, this.Const.World.Spawn.CaravanSouthern, this.m.Start.getResources() * 0.6);
		}
		else
		{
			party = _faction.spawnEntity(this.m.Start.getTile(), "Karawana Kupiecka", false, this.Const.World.Spawn.Caravan, this.m.Start.getResources() * 0.5);
		}

		party.getSprite("banner").Visible = false;
		party.getSprite("base").Visible = false;
		party.setMirrored(true);
		party.setDescription("Karawana kupiecka z " + this.m.Start.getName() + ", przewożąca wszelakie towary między osadami.");

this.send_refugees_action <- this.inherit("scripts/factions/faction_action", {
	...
	{
		local party = _faction.spawnEntity(this.m.Start.getTile(), "UchodЄcy", false, this.Const.World.Spawn.Peasants, this.Math.rand(30, 60));
		party.getSprite("banner").Visible = false;
		party.getSprite("body").setBrush("figure_civilian_0" + this.Math.rand(1, 2));
		party.setDescription("Uchodźcy uciekający przez okropieństwami wojny - rozbici, wycieńczeni i zdesperowani.");

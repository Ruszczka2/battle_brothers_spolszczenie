this.send_nomad_roamers_action <- this.inherit("scripts/factions/faction_action", {
	...
	function onExecute( _faction )
	{
		...
		local party = this.getFaction().spawnEntity(settlement.getTile(), "Koczownicy", false, this.Const.World.Spawn.NomadRoamers, this.Math.min(settlement.getResources(), this.Math.rand(60, 110)));
		party.getSprite("banner").setBrush(settlement.getBanner());
		party.setDescription("Grupa koczowników dokonująca zwiadu okolicy.");

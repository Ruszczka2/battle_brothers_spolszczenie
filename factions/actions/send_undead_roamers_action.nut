this.send_undead_roamers_action <- this.inherit("scripts/factions/faction_action", {
	...
	function onExecute( _faction )
	{
		...
		local party = this.getFaction().spawnEntity(settlement.getTile(), "Nieumarli", false, settlement.getRoamerSpawnList(), this.Math.max(settlement.getResources() * 0.75, this.Math.rand(60, 100) * this.getReputationToDifficultyLightMult()));

this.send_bandit_ambushers_action <- this.inherit("scripts/factions/faction_action", {
	...
	function onExecute( _faction )
	{
		...
		local party = this.getFaction().spawnEntity(settlement.getTile(), "Bandyci", false, this.Const.World.Spawn.BanditRaiders, this.Math.rand(75, 120) * this.getReputationToDifficultyLightMult() * mult);
		party.getSprite("banner").setBrush(settlement.getBanner());
		party.setDescription("Szajka zatwardziałych bandytów żerująca na słabszych.");

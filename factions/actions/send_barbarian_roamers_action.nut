this.send_barbarian_roamers_action <- this.inherit("scripts/factions/faction_action", {
	...
	function onExecute( _faction )
	{
		...
		local party = this.getFaction().spawnEntity(settlement.getTile(), "Barbarzyńcy", false, this.Const.World.Spawn.BarbarianHunters, this.Math.min(settlement.getResources(), this.Math.rand(60, 110)));
		party.getSprite("banner").setBrush(settlement.getBanner());
		party.setDescription("Grupa barbarzyńców polująca na zwierzynę.");

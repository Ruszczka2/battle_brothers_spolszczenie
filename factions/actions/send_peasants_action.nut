this.send_peasants_action <- this.inherit("scripts/factions/faction_action", {
	...
	function onExecute( _faction )
	{
		...
		{
			party = _faction.spawnEntity(this.m.Start.getTile(), "Obywatele", false, this.Const.World.Spawn.PeasantsSouthern, this.Math.rand(30, 60));
			party.getSprite("body").setBrush("figure_civilian_06");
			party.setDescription("Rolnicy, rzemieślnicy, pielgrzymi i inni obywatele wielkich państw-miast w drodze między osadami.");
		}
		else
		{
			party = _faction.spawnEntity(this.m.Start.getTile(), "ChՄopstwo", false, this.Const.World.Spawn.Peasants, this.Math.rand(30, 60));
			party.getSprite("body").setBrush("figure_civilian_0" + this.Math.rand(1, 5));
			party.setDescription("Rolnicy, rzemieślnicy, pielgrzymi i inni chłopi w drodze między osadami.");
		}

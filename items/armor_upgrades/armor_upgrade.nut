this.armor_upgrade <- this.inherit("scripts/items/item", {
	...
	function getTooltip()
	{
		...
		result.push({
			id = 65,
			type = "text",
			text = "Kliknij prawym przyciskiem myszy lub przeciągnij na zbroję obecnie wybranej postaci, aby trwale połączyć. Ten przedmiot zostanie zużyty w trakcie łączenia i da zbroi następujące efekty:"
		});
		return result;
	}
	...
	function getArmorTooltip( _result )
	{
		...
		_result.push({
			...
			text = "Posiada dodatek, który można wymienić, ale dawny dodatek zostanie zniszczony podczas zamiany"
		});
	}

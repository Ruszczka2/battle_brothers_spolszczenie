this.serpent_skin_upgrade <- this.inherit("scripts/items/armor_upgrades/armor_upgrade", {
	...
	function create()
	{
		...
		this.m.Name = "Opończa ze Skóry Węża";
		this.m.Description = "Opończa wykonana z cienkich i połyskujących łusek pustynnych węży, szczególnie odporna na żar i płomienie.";
		this.m.ArmorDescription = "Opończa ze skóry węża została przyczepiona do tej zbroi, czyniąc ją bardziej odporną na żar i płomienie.";
	...
	function getTooltip()
	{
		...
		result.push({
			...
			text = "Redukuje obrażenia od ognia i broni palnej o [color=" + this.Const.UI.Color.NegativeValue + "]33%[/color]"
		});
		return result;
	}

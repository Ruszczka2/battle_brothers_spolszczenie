this.large_powder_bag <- this.inherit("scripts/items/ammo/ammo", {
	...
	function create()
	{
		...
		this.m.Name = "Duży Worek Prochu";
		this.m.Description = "Duży worek z czarnym prochem, używanym do nabijania egzotycznej broni palnej. Zostaje automatycznie uzupełniony po każdej bitwie, jeżeli masz wystarczającą ilość amunicji.";
	...
	function getTooltip()
	{
		...
		if (this.m.Ammo != 0)
		{
			result.push({
				...
				text = "Zawiera [color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.Ammo + "[/color] ładunków prochu"
			});
		}
		else
		{

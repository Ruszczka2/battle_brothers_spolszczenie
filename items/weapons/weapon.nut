this.weapon <- this.inherit("scripts/items/item", {
	...
	function getTooltip()
	{
		...
		if (this.m.RegularDamage > 0)
		{
			result.push({
				...
				text = "Obrażenia [color=" + this.Const.UI.Color.DamageValue + "]" + this.m.RegularDamage + "[/color] - [color=" + this.Const.UI.Color.DamageValue + "]" + this.m.RegularDamageMax + "[/color]"
			});
		}
		...
		if (this.m.DirectDamageMult > 0)
		{
			result.push({
				...
				text = "[color=" + this.Const.UI.Color.DamageValue + "]" + this.Math.floor((this.m.DirectDamageMult + this.m.DirectDamageAdd) * 100) + "%[/color] obrażeń ignoruje pancerz"
			});
		}
		...
		if (this.m.ArmorDamageMult > 0)
		{
			result.push({
				...
				text = "[color=" + this.Const.UI.Color.DamageValue + "]" + this.Math.floor(this.m.ArmorDamageMult * 100) + "%[/color] skuteczności przeciwko pancerzowi"
			});
		}
		...
		if (this.m.ShieldDamage > 0)
		{
			result.push({
				...
				text = "Obrażenia wobec tarcz [color=" + this.Const.UI.Color.DamageValue + "]" + this.m.ShieldDamage + "[/color]"
			});
		}
		...
		if (this.m.ChanceToHitHead > 0)
		{
			result.push({
				...
				text = "Szansa trafienia w głowę [color=" + this.Const.UI.Color.PositiveValue + "]+" + this.m.ChanceToHitHead + "%[/color]"
			});
		}
		...
		if (this.m.AdditionalAccuracy > 0)
		{
			result.push({
				...
				text = "Ma dodatkowe [color=" + this.Const.UI.Color.PositiveValue + "]+" + this.m.AdditionalAccuracy + "%[/color] szansy na trafienie"
			});
		}
		else if (this.m.AdditionalAccuracy < 0)
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma dodatkowe [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.AdditionalAccuracy + "%[/color] szansy na trafienie"
			});
		}
		...
		if (this.m.RangeMax > 1)
		{
			result.push({
				...
				text = "Zasięg [color=" + this.Const.UI.Color.PositiveValue + "]" + this.getRangeMax() + "[/color] pól"
			});
		}
		...
		if (this.m.FatigueOnSkillUse > 0 && (this.getContainer() == null || !this.getContainer().getActor().getCurrentProperties().IsProficientWithHeavyWeapons))
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "Umiejętności broni generują o [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.FatigueOnSkillUse + "[/color] więcej zmęczenia"
			});
		}
		else if (this.m.FatigueOnSkillUse < 0)
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "Umiejętności broni generują o [color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.FatigueOnSkillUse + "[/color] mniej zmęczenia"
			});
		}
		...
		if (this.m.AmmoMax > 0)
		{
			if (this.m.Ammo != 0)
			{
				result.push({
					...
					text = "Zawiera amunicję na [color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.Ammo + "[/color] użyć"
				});
			}
			else
			{
				result.push({
					...
					text = "[color=" + this.Const.UI.Color.NegativeValue + "]Jest pusta i bezużyteczna[/color]"
				});
			}
		}
	...
	function lowerCondition( _value = this.Const.Combat.WeaponDurabilityLossOnHit )
	{
		...
		{
			this.Tactical.EventLog.log(this.getName() + " postaci " + this.Const.UI.getColorizedEntityName(actor) + " ulega zniszczeniu!");

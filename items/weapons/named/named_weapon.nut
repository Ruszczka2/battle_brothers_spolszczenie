this.named_weapon <- this.inherit("scripts/items/weapons/weapon", {
	...
	function createRandomName()
	{
		if ((!this.m.UseRandomName || this.Math.rand(1, 100) <= 60) && this.m.PrefixList.len() + this.m.SuffixList.len() > 0)
		{
			...
			{
				return this.Const.Strings.RandomWeaponPrefix[this.Math.rand(0, this.Const.Strings.RandomWeaponPrefix.len() - 1)] + " " + this.m.PrefixList[this.Math.rand(0, this.m.PrefixList.len() - 1)];
			}
			else
			{
				return this.Const.Strings.RandomWeaponPrefixMale[this.Math.rand(0, this.Const.Strings.RandomWeaponPrefixMale.len() - 1)] + " " + this.m.SuffixList[this.Math.rand(0, this.m.SuffixList.len() - 1)];
			}
		}

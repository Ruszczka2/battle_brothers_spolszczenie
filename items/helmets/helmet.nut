this.helmet <- this.inherit("scripts/items/item", {
	...
	function getTooltip()
	{
		...
		if (this.m.Vision < 0)
		{
			result.push({
				...
				text = "Wzrok [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.Vision + "[/color]"
			});
		}
		else if (this.m.Vision > 0)
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/vision.png",
				text = "Wzrok [color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.Vision + "[/color]"
			});
		}
	...
	function onDamageReceived( _damage, _fatalityType, _attacker )
	{
		...
		{
			this.Tactical.EventLog.logEx(this.getName() + " postaci " + this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " otrzymuje [b]" + this.Math.floor(_damage) + "[/b] obrażeń i ulega zniszczeniu!");
		...
		{
			this.Tactical.EventLog.logEx(this.getName() + " postaci " + this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " otrzymuje [b]" + this.Math.floor(_damage) + "[/b] obrażeń");
		}

this.named_helmet <- this.inherit("scripts/items/helmets/helmet", {
	...
	function createRandomName()
	{
		if ((!this.m.UseRandomName || this.Math.rand(1, 100) <= 60) && this.m.PrefixList.len() + this.m.SuffixList.len() > 0)
		{
			...
			{
				return this.Const.Strings.RandomArmorPrefix[this.Math.rand(0, this.Const.Strings.RandomArmorPrefix.len() - 1)] + " " + this.m.PrefixList[this.Math.rand(0, this.m.PrefixList.len() - 1)];
			}
			else
			{
				return this.Const.Strings.RandomArmorPrefixMale[this.Math.rand(0, this.Const.Strings.RandomArmorPrefixMale.len() - 1)] + " " + this.m.SuffixList[this.Math.rand(0, this.m.SuffixList.len() - 1)];
			}
		}

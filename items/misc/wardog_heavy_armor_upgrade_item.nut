this.wardog_heavy_armor_upgrade_item <- this.inherit("scripts/items/item", {
	...
	function create()
	{
		...
		this.m.Name = "Ciężki Psi Pancerz";
		this.m.Description = "Ciężki skórzany płaszcz, w który można przyodziać dowolnego pas wojennego, aby zapewnić mu ochronę w walce.";
	...
	function getTooltip()
	{
		...
		result.push({
			id = 65,
			type = "text",
			text = "Kliknij prawym przyciskiem myszy lub przeciągnij na psa obecnie wybranej postaci, aby użyć. Przedmiot zostanie zużyty po wykorzystaniu."
		});
		return result;
	}

this.necromancer_potion_item <- this.inherit("scripts/items/misc/anatomist/anatomist_potion_item", {
	...
	function create()
	{
		...
		this.m.Name = "Król w Okowach";
		this.m.Description = "Z wielu broni dostępnych dla człowieka, najpotężniejszą jest bez wątpienia jego umysł, i po nikim nie widać tego bardziej, co po budzącym postrach nekromancie. Dzięki tej miksturze, ten przerażający intelekt jest w zasięgu ręki nawet przeciętnego człowieka!";
		this.m.IconLarge = "";
	...
	function getTooltip()
	{
		...
		result.push({
			...
			text = "[color=" + this.Const.UI.Color.PositiveValue + "]+20%[/color] do zdobywanego doświadczenia"
		});
		result.push({
			id = 65,
			type = "text",

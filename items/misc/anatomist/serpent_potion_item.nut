this.serpent_potion_item <- this.inherit("scripts/items/misc/anatomist/anatomist_potion_item", {
	...
	function create()
	{
		...
		this.m.Name = "Mikstura Szybkiego Kła";
		this.m.Description = "Dzięki tej miksturze, budzący postrach spryt i zabójcza precyzja nienawistnych pustynnych żmij są na wyciagnięcie ręki każdego wojownika!";
		this.m.IconLarge = "";
	...
	function getTooltip()
	{
		...
		result.push({
			...
			text = "Zyskuje [color=" + this.Const.UI.Color.PositiveValue + "]+3%[/color] do szansy trafienia w walce wręcz za każdego sojusznika przyległego do twego celu"
		});
		result.push({
			id = 65,
			type = "text",

this.orc_berserker_potion_item <- this.inherit("scripts/items/misc/anatomist/anatomist_potion_item", {
	...
	function create()
	{
		...
		this.m.Name = "Różany Napój Miłosny";
		this.m.Description = "Ten napój, wytworzony z gruczołów adrenalinowych tak zwanych Orków \'Berzerkerów\', zawdzięcza swą nazwę wprowadzaniu pijącego w podwyższony stan emocjonalny oraz powodowaniu rumieńców.";
		this.m.IconLarge = "";
	...
	function getTooltip()
	{
		...
		result.push({
			...
			text = "Przy otrzymywaniu obrażeń Zdrowia kumuluje Gniew, który zwiększa Stanowczość, Inicjatywę i zadawane obrażenia, a także obniża obrażenia otrzymywane"
		});
		result.push({
			id = 65,
			type = "text",

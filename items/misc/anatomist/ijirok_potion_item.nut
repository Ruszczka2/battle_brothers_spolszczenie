this.ijirok_potion_item <- this.inherit("scripts/items/misc/anatomist/anatomist_potion_item", {
	...
	function create()
	{
		...
		this.m.Name = "Eliksir Szalonego Boga";
		this.m.Description = "Główna etykieta na tym eliksirze jest nieczytelna, składa się z pospiesznie nabazgranych znaków w jakimś obcym lub zmyślonym języku. Pod nią, drobnym druczkiem, napisano: \'EFEKTY UBOCZNE: LICZNE\'. Fascynujące!";
		this.m.IconLarge = "";
	...
	function getTooltip()
	{
		...
		result.push({
			...
			text = "[color=" + this.Const.UI.Color.PositiveValue + "]50%[/color] szansy, aby odeprzeć efekty takie jak np. Oszołomienie czy Ogłuszenie"
		});
		result.push({
			id = 65,
			type = "text",

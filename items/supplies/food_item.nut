	...
	function getTooltip()
	{
		...
		if (this.m.BoughtAtPrice > 0)
		{
			result.push({
				id = 7,
				type = "text",
				text = "Kupiono za [img]gfx/ui/tooltips/money.png[/img]" + this.m.BoughtAtPrice
			});
		}
		...
		result.push({
			id = 67,
			type = "text",
			text = "Zepsuje się w ciągu [color=" + this.Const.UI.Color.NegativeValue + "]" + this.getSpoilInDays() + "[/color] dni."
		});
		return result;
	}

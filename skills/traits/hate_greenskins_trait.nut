this.hate_greenskins_trait <- this.inherit("scripts/skills/traits/character_trait", {
	...
	function create()
	{
		...
		this.m.Name = "Nienawiść do Zielonoskórych";
		this.m.Icon = "ui/traits/trait_icon_52.png";
		this.m.Description = "Jakieś minione wydarzenie w życiu tej postaci zapoczątkowało jej zagorzałą nienawiść do wszelkich zielonych i podłych istot.";
		this.m.Titles = [
			"Orkobójca"
		];
		this.m.Excluded = [
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.PositiveValue + "]+10[/color] do Stanowczości podczas bitwy z zielonoskórymi"
			}
		];
	}

this.hate_undead_trait <- this.inherit("scripts/skills/traits/character_trait", {
	...
	function create()
	{
		...
		this.m.Name = "Nienawiść do Nieumarłych";
		this.m.Icon = "ui/traits/trait_icon_50.png";
		this.m.Description = "Jakieś minione wydarzenie w życiu tej postaci zapoczątkowało jej zagorzałą nienawiść do wszelkich istot, które nie zamierzają po dobroci pozostać sześć stóp pod ziemią.";
		this.m.Excluded = [
			"trait.weasel",
			"trait.craven",
			"trait.dastard",
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.PositiveValue + "]+10[/color] do Stanowczości podczas bitwy z nieumarłymi"
			}
		];
	}

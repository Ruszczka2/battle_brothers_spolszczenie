this.hate_beasts_trait <- this.inherit("scripts/skills/traits/character_trait", {
	...
	function create()
	{
		...
		this.m.Name = "Nienawiść do Bestii";
		this.m.Icon = "ui/traits/trait_icon_51.png";
		this.m.Description = "Jakieś minione wydarzenie w życiu tej postaci zapoczątkowało jej zagorzałą nienawiść do wszelkich dzikich i potwornych istot.";
		this.m.Excluded = [
			"trait.weasel",
			"trait.craven",
			"trait.dastard",
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.PositiveValue + "]+10[/color] do Stanowczości podczas bitwy z bestiami"
			}
		];
	}

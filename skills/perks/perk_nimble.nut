this.perk_nimble <- this.inherit("scripts/skills/skill", {
	...
	{
		return "Zwinny niczym kot! Ta postać jest w stanie częściowo uniknąć lub odbić ataki w ostatniej chwili, zamieniając je w draśnięcia. Im lżejszy pancerz, tym bardziej na tym skorzystasz.";
	}
	...
	function getTooltip()
	{
		...
		if (fm < 100)
		{
			tooltip.push({
				...
				text = "Otrzymuje jedynie [color=" + this.Const.UI.Color.PositiveValue + "]" + fm + "%[/color] obrażeń do Zdrowia od ataków"
			});
		}
		else
		{
			tooltip.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Zbroja i hełm tej postaci są zbyt ciężkie, aby mogła ona odnieść korzyści z talentu Zwinny[/color]"
			});
		}

this.cut_artery_injury <- this.inherit("scripts/skills/injury/injury", {
	...
	function create()
	{
		...
		this.m.Name = "Przecięta Tętnica";
		this.m.Description = "Tętnica została przecięta, powodując masywny krwotok i co ostatecznie zakończy się śmiercią, jeśli rany nie opatrzy się należycie poza walką. Jeżeli postać przeżyje, będzie miała obniżoną wytrzymałość z uwagi na utratę krwi.";
	...
	function getTooltip()
	{
		...
		if (!this.m.IsShownOutOfCombat)
		{
			ret.push({
				...
				text = "Ta postać będzie w walce tracić [color=" + this.Const.UI.Color.NegativeValue + "]3[/color] punkty zdrowia co turę"
			});
		}
		else
		{

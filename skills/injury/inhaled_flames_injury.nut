this.inhaled_flames_injury <- this.inherit("scripts/skills/injury/injury", {
	...
	function create()
	{
		...
		this.m.Name = "Poparzony Przewód Oddechowy";
		this.m.Description = "Wdychanie płomieni nie było zbyt dobry pomysłem. Poparzony przewód oddechowy sprawia, że każda próba złapania powietrza jest niezwykle bolesna.";
	...
	function getTooltip()
	{
		local ret = [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-2[/color] do Punktów Akcji"
			},
			{
				id = 7,
				type = "text",

this.shoot_bolt <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Wystrzel Bełt";
		this.m.Description = "Szybkie pociągnięcie za spust, by wypuścić bełt. Kusza musi zostać przeładowana po każdym strzale, by znów móc z niej wystrzelić.";
	...
	function getTooltip()
	{
		...
		if (15 + this.m.AdditionalAccuracy >= 0)
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]+" + (15 + this.m.AdditionalAccuracy) + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-3 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma [color=" + this.Const.UI.Color.NegativeValue + "]" + (15 + this.m.AdditionalAccuracy) + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-3 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}

this.hex_skill <- this.inherit("scripts/skills/skill", {
	...
	function onDelayedEffect( _tag )
	{
		...
		if (target.getCurrentProperties().IsResistantToAnyStatuses && this.Math.rand(1, 100) <= 50)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(target) + " odpiera klątwę dzięki swej niezwykłej fizjologii");
			}
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " rzuca klątwę na: " + this.Const.UI.getColorizedEntityName(target));

this.throw_axe <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Rzuć Toporem";
		this.m.Description = "Rzuć toporem w cel. Umiejętność nie może być użyta, gdy postać sąsiaduje z wrogiem walczącym wręcz.";
	...
	function getTooltip()
	{
		...
		if (ammo > 0)
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]" + ammo + "[/color] toporów w zapasie"
			});
		}
		else
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Brak toporów[/color]"
			});
		}

this.quick_shot <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Szybki Strzał";
		this.m.Description = "Szybki intuicyjny strzał, gdy nie ma czasu na pełny naciąg, kotwiczenie i celowanie. Celność drastycznie spada wraz z odległością. Umiejętność nie może być użyta, gdy postać sąsiaduje z wrogiem walczącym wręcz.";
		this.m.KilledString = "Zastrzelony z łuku";
	...
	function getTooltip()
	{
		...
		if (this.m.AdditionalAccuracy >= 0)
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]+" + this.m.AdditionalAccuracy + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-4 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.AdditionalAccuracy + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-4 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		...
		if (ammo > 0)
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]" + ammo + "[/color] strzał w zapasie"
			});
		}
		else
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Wymaga założonego, nie-pustego kołczanu[/color]"
			});
		}

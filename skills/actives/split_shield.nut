this.split_shield <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Rozłup Tarczę";
		this.m.Description = "Atak wymierzony konkretnie w to, aby zniszczyć tarczę celu. Można go użyć tylko przeciwko celom dzierżącym tarczę. Zawsze trafi, choć rozłupanie tarczy może wymagać kilku ciosów, w zależności od rodzaju tarczy i użytej broni.";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "Zadaje [color=" + this.Const.UI.Color.DamageValue + "]" + damage + "[/color] obrażeń tarczom"
		});
		...
		if (this.m.MaxRange > 1)
		{
			ret.push({
				...
				text = "Ma zasięg [color=" + this.Const.UI.Color.PositiveValue + "] " + this.m.MaxRange + "[/color] pól"
			});
		}
	...
	function onUse( _user, _targetTile )
	{
		...
		if (shield != null)
		{
			...
			if (shield.getCondition() == 0)
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " używa Rozłupania Tarczy i niszczy tarczę, którą dzierży " + this.Const.UI.getColorizedEntityName(target));
				}
			}
			else
			{
				...
				if (!_user.isHiddenToPlayer() && _targetTile.IsVisibleForPlayer)
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " używa Rozłupania Tarczy i zadaje [b]" + (conditionBefore - shield.getCondition()) + "[/b] obrażeń tarczy, którą dzierży " + this.Const.UI.getColorizedEntityName(target));
				}
			}

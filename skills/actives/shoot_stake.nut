this.shoot_stake <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Wystrzel Ciężki Bełt";
		this.m.Description = "Szybkie pociągnięcie za spust, by wypuścić ciężki bełt. Kusza musi zostać przeładowana po każdym strzale, by znów móc z niej wystrzelić.";
		this.m.KilledString = "Zastrzelony z kuszy";
	...
	function getTooltip()
	{
		...
		if (10 + this.m.AdditionalAccuracy >= 0)
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]+" + (10 + this.m.AdditionalAccuracy) + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-3 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma [color=" + this.Const.UI.Color.NegativeValue + "]" + (10 + this.m.AdditionalAccuracy) + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-3 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		...
		if (ammo > 0)
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]" + ammo + "[/color] bełtów w zapasie"
			});
		}
		else
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Wymaga założonego kołczanu z bełtami, który nie jest pusty[/color]"
			});
		}
	...
	function onTargetHit( _skill, _targetEntity, _bodyPart, _damageInflictedHitpoints, _damageInflictedArmor )
	{
		if (_skill == this && _targetEntity.isAlive() && !_targetEntity.isDying())
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(user) + " odrzuca w tył: " + this.Const.UI.getColorizedEntityName(_targetEntity));
			}

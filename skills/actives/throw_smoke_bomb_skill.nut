this.throw_smoke_bomb_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Rzuć Dymny Garnuszek";
		this.m.Description = "Rzuć garnuszkiem wypełnionym substancjami, które po uderzeniu szybko utworzą gęstą chmurę.";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			type = "text",
			icon = "ui/icons/special.png",
			text = "Zwiększa Obronę dystansową o [color=" + this.Const.UI.Color.PositiveValue + "]+100%[/color], ale pomniejsza Atak dystansowy o [color=" + this.Const.UI.Color.NegativeValue + "]-50%[/color] dla każdej postaci wewnątrz chmury"
		});
		ret.push({
			id = 6,
			type = "text",
			icon = "ui/icons/special.png",
			text = "Ugasza obecny efekt pola, taki jak Ogień lub Wyziewy"
		});
		return ret;
	}
	...
	function onApply( _data )
	{
		...
		local p = {
			Type = "smoke",
			Tooltip = "Gesty dym pokrywa ten teren, umożliwiając każdej postaci znajdującej się w chmurze na swobodny ruch i ignorowanie stref kontroli",
			IsPositive = true,

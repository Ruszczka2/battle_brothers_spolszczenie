this.pound <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Utłucz";
		this.m.Description = "Wbicie wroga w ziemię przy użyciu łańcucha i metalowej głowicy. Dość nieprzewidywalne jeśli chodzi o obrażenia, ale może uderzyć ponad lub wokół tarczy. Trafiony cel może zostać ogłuszony na jedną turę.";
		this.m.KilledString = "Roztarty na miazgę";
	...
	function getTooltip()
	{
		...
		if (p.IsSpecializedInFlails)
		{
			ret.push({
				...
				text = "Trafienia w głowę ignorują dodatkowe [color=" + this.Const.UI.Color.PositiveValue + "]20%[/color] pancerza"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/special.png",
				text = "Trafienia w głowę ignorują dodatkowe [color=" + this.Const.UI.Color.PositiveValue + "]10%[/color] pancerza"
			});
		}

this.throw_fire_bomb_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Rzuć Ognisty Garnuszek";
		this.m.Description = "Podpal i rzuć w kierunku celu garnuszek wypełniony łatwopalnym płynem. Garnuszek rozbije się i pokryje okolicę płomieniami. Każda postać kończąca swoją turę na podpalonym terenie również zostanie podpalona i otrzyma obrażenia - bez względu na to, czy jest to wróg, czy przyjaciel.";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "Podpala na 2 rundy teren [color=" + this.Const.UI.Color.DamageValue + "]7[/color] pól. Wody i śniegu nie można podpalić."
		});
		ret.push({
			id = 6,
			type = "text",
			icon = "ui/icons/special.png",
			text = "Wypala istniejący efekt na polu, taki jak Dym lub Wyziewy"
		});
		return ret;
	}
	...
	function onApply( _data )
	{
		...
		local p = {
			Type = "fire",
			Tooltip = "Szaleje tu ogień, topiący zarówno pancerz, jak i ciało",
			IsPositive = false,

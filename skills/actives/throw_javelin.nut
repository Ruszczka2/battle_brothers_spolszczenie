this.throw_javelin <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Rzuć Oszczepem";
		this.m.Description = "Rzuć oszczepem we wroga. Umiejętność nie może być użyta, gdy postać sąsiaduje z wrogiem walczącym wręcz.";
		this.m.KilledString = "Przeszyty oszczepem";
	...
	function getTooltip()
	{
		...
		if (30 + this.m.AdditionalAccuracy >= 0)
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]+" + (30 + this.m.AdditionalAccuracy) + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-10 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma [color=" + this.Const.UI.Color.NegativeValue + "]" + (30 + this.m.AdditionalAccuracy) + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-10 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		...
		if (ammo > 0)
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]" + ammo + "[/color] oszczepów w zapasie"
			});
		}
		else
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Brak oszczepów[/color]"
			});
		}

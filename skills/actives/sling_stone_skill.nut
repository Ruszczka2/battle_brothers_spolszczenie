this.sling_stone_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Miotnij Kamieniem";
		this.m.Description = "Poślij kamień w stronę celu korzystając ze swojej procy. Celność mocno spada z odległością. Umiejętność nie może być użyta, gdy postać sąsiaduje z wrogiem walczącym wręcz.";
		this.m.KilledString = "Ukamienowany";
	...
	function getTooltip()
	{
		...
		ret.extend([
			{
				...
				text = "Ma zasięg [color=" + this.Const.UI.Color.PositiveValue + "]" + this.getMaxRange() + "[/color] pól na równym terenie, więcej strzelając ze wzniesienia"
			}
		]);
		...
		if (this.m.AdditionalAccuracy == 0)
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.NegativeValue + "]" + (-5 + this.m.AdditionalHitChance) + "%[/color] szansy na trafienie na każde pole odległości"
			});
		}
		else if (this.m.AdditionalAccuracy > 0)
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]+" + this.m.AdditionalAccuracy + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-10 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.AdditionalAccuracy + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-10 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		...
		ret.push({
			...
			text = "Ma [color=" + this.Const.UI.Color.NegativeValue + "]100%[/color] szansę na oszołomienie celu po trafieniu w głowę"
		});
	...
	function onTargetHit( _skill, _targetEntity, _bodyPart, _damageInflictedHitpoints, _damageInflictedArmor )
	{
		if (_skill == this && _targetEntity.isAlive() && !_targetEntity.isDying())
		{
			...
			if (_bodyPart == this.Const.BodyPart.Head && !_targetEntity.getCurrentProperties().IsImmuneToDaze)
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(user) + " trafia w głowę, przez co " + this.Const.UI.getColorizedEntityName(_targetEntity) + " jest oszołomiony");
				}
			}
		}
	}

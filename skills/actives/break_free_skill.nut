this.break_free_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Uwolnij Się";
		this.m.Description = "Użyj wszystkiego, co masz do dyspozycji, aby się z tego, co uwięziło cię w miejscu. Rąb, siekaj, tnij lub nawet gryź, jeśli zajdzie potrzeba!";
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]" + this.getChance() + "%[/color] szansy na powodzenie, bazując na Ataku w zwarciu. Szansa jest wyższa, jeśli już próbowano uwolnić cel."
			}
		];
	}
	...
	function onUse( _user, _targetTile )
	{
		...
		if (rolled <= toHit)
		{
			this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(_user) + " uwalnia się (Szansa: " + toHit + ", Rzut: " + rolled + ")");
		...
		{
			this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(_user) + " nie daje rady się uwolnić (Szansa: " + toHit + ", Rzut: " + rolled + ")");

this.throw_spear_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Rzuć Włócznią";
		this.m.Description = "Rzuć włócznią w kierunku celu, aby uczynić jego tarczę bezużyteczną. Cele posiadające tarczę zawsze zostaną trafione. Trafienie celu cez tarczy może zadać niszczycielskie obrażenia, choć atak może chybić. Umiejętność nie może być użyta, gdy postać sąsiaduje z wrogiem walczącym wręcz.";
	...
	function onApplyShieldDamage( _tag )
	{
		...
		if (_tag.Shield.getCondition() == 0)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_tag.User) + " niszczy tarczę, którą dzierży " + this.Const.UI.getColorizedEntityName(_tag.TargetTile.getEntity()));
			}
		}
		else
		{
			if (!_tag.User.isHiddenToPlayer() && _tag.TargetTile.IsVisibleForPlayer)
			{
				this.Tactical.EventLog.log("Tarcza, która dzierży " + this.Const.UI.getColorizedEntityName(_tag.TargetTile.getEntity()) + " otrzymuje [b]" + (conditionBefore - _tag.Shield.getCondition()) + "[/b] obrażeń");
			}

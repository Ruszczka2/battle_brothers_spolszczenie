this.lunge_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Natarcie";
		this.m.Description = "Szybkie natarcie na cel oddalony o 2 pola, po którym zadajesz precyzyjne pchnięcie, póki wróg nie jest przygotowany. Im szybszy jesteś, tym więcej obrażeń zadasz.";
		this.m.KilledString = "Poszatkowany";
	...
	function getTooltip()
	{
		...
		ret.extend([
			{
				...
				text = "Przemieszcza używającego w pobliże celu, ignorując Strefę Kontroli"
			},
			{
				id = 6,
				type = "text",
				icon = "ui/icons/special.png",
				text = "Im wyższa obecna Inicjatywa postaci, tym wyższe obrażenia dodatkowe"
			}
		]);
		...
		if (this.getContainer().getActor().getCurrentProperties().IsRooted)
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Nie można użyć będąc zakorzenionym[/color]"
			});
		}
	...
	function onTeleportDone( _entity, _tag )
	{
		...
		foreach( actor in ZOC )
		{
			...
			if (actor.onAttackOfOpportunity(_entity, true))
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_entity) + " naciera i zostaje odparty");
				}

this.gash_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Rozetnij";
		this.m.Description = "Celnie wymierzony atak cięty, który prawdopodobnie zada niszczycielskie rany.";
		this.m.KilledString = "Zaszlachtowany";
	...
	function getTooltip()
	{
		...
		if (this.getContainer().getActor().getCurrentProperties().IsSpecializedInSwords)
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma o [color=" + this.Const.UI.Color.NegativeValue + "]50%[/color] niższy próg na zadanie kontuzji"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma o [color=" + this.Const.UI.Color.NegativeValue + "]33%[/color] niższy próg na zadanie kontuzji"
			});
		}

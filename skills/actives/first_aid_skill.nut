this.first_aid_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Pierwsza Pomoc";
		this.m.Description = "Ocal się przed wykrwawieniem się na śmierć, uciskając i prowizorycznie bandażując ranę. Nie leczy punktów zdrowia. Umiejętność nie może być użyta, gdy postać sąsiaduje z wrogiem walczącym wręcz.";
	...
	function getTooltip()
	{
		...
		if (!isQualified)
		{
			ret.push({
				...
				type = "text",
				icon = "ui/tooltips/warning.png",
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Można użyć tylko podczas krwawienia[/color]"
			});
		}

	...
	function onDelayedEffect( _tag )
	{
		...
		foreach( target in targets )
		{
			...
			if (target.checkMorale(0, -35 * bonus, this.Const.MoraleCheckType.MentalAttack))
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(target) + " opiera się pragnieniu snu dzięki swej stanowczości");
				}
			...
			if (!_user.isHiddenToPlayer() && !target.isHiddenToPlayer())
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(target) + " zapada w sen");
			}
		}
	}

this.aimed_shot <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Mierzony Strzał";
		this.m.Description = "Skupiony i dokładnie wymierzony strzał z dużą szansą trafienia pożądanego celu, nawet na sporą odległość. Umiejętność nie może być użyta, gdy postać sąsiaduje z wrogiem walczącym wręcz.";
	...
	function getTooltip()
	{
		...
		if (10 + this.m.AdditionalAccuracy >= 0)
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]+" + (10 + this.m.AdditionalAccuracy) + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-2 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma [color=" + this.Const.UI.Color.NegativeValue + "]" + (10 + this.m.AdditionalAccuracy) + "%[/color] szansy na trafienie i [color=" + this.Const.UI.Color.NegativeValue + "]" + (-2 + this.m.AdditionalHitChance) + "%[/color] na każde pole odległości"
			});
		}
		...
		else
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Wymaga założonego kołczanu, który nie jest pusty[/color]"
			});
		}

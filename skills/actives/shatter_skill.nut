this.shatter_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Roztrzaskaj";
		this.m.Description = "Wykonaj bronią zamach po szerokim łuku, mogąc trafić trzy sąsiadujące cele w kolejności przeciwnej do ruchu wskazówek zegara. Trafiony cel może zostać odrzucony w tył lub zachwiać się od siły uderzenia. Ostrożnie z używaniem tej zdolności w pobliżu własnych ludzi, chyba że chcesz zmniejszyć swoją listę płac!";
		...
		ret.extend([
			{
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]33%[/color] szansy na wytrącenie celu z równowagi"
			},
			{
				id = 9,
				type = "text",
				icon = "ui/icons/special.png",
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]33%[/color] szansy na odrzucenie celu w tył"
			},
			{
				id = 6,
				type = "text",
	...
	function applyEffectToTarget( _user, _target, _targetTile )
	{
		...
		else if (applyEffect == 2)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " odrzuca w tył: " + this.Const.UI.getColorizedEntityName(_target));
			}

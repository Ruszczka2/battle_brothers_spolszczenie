this.strike_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Uderz";
		this.m.Description = "Zamaszyste uderzenie, które sięga aż na 2 pola i może zostać użyte zza przedniego szeregu, będąc poza zasięgiem większości broni białej.";
		...
		if (this.m.ApplyAxeMastery)
		{
			if (!this.getContainer().getActor().getCurrentProperties().IsSpecializedInAxes)
			{
				ret.push({
					...
					text = "Ma [color=" + this.Const.UI.Color.NegativeValue + "]" + this.getHitChanceModifier() + "%[/color] szans na trafienie celów bezpośrednio sąsiadujących"
				});
			}
		}
		else

this.hook <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Zahacz";
		this.m.Description = "Cel oddalony o 2 pola zostaje zahaczony i przyciągnięty bliżej, jeżeli jest miejsce. Każda trafiona postać zostanie wytrącona z równowagi i straci inicjatywę. Cel nie może zostać wciągnięty pod górę, lecz może otrzymać obrażenia, jeśli spadnie z dużej wysokości. Ściana Tarcz, Ściana Włóczni i Riposta celu zostaną anulowane przy udanym zahaczeniu. Ukorzeniony cel nie może zostać zahaczony.";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]100%[/color] szansy na wytrącenie z równowagi przy trafieniu"
		});
		ret.push({
			id = 6,
			type = "text",
	...
	function onUse( _user, _targetTile )
	{
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " zahacza: " + this.Const.UI.getColorizedEntityName(_targetTile.getEntity()));
		}

	...
	function onDelayedEffect( _tag )
	{
		...
		this.Time.scheduleEvent(this.TimeUnit.Virtual, time, function ( _e )
		{
			...
			if (target.checkMorale(0, -35 + bonus, this.Const.MoraleCheckType.MentalAttack))
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(target) + " odpiera urok dzięki swej stanowczości");
				}
			...
			if (!_user.isHiddenToPlayer() && !target.isHiddenToPlayer())
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(target) + " jest pod wpływem uroku");
			}

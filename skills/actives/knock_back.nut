this.knock_back <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Odepchnij";
		this.m.Description = "Użyj tarczy, aby odepchnąć cel o jedno pole. Trafiony cel nieco się zmęczy, a do tego może otrzymać obrażenia, jeśli spadnie z dużej wysokości. Ściana Tarcz, Ściana Włóczni i Riposta celu zostaną anulowane przy udanym odepchnięciu. Ukorzeniony cel nie może zostać odepchnięty.";
	...
	function getTooltip()
	{
		...
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]+25%[/color] szansy na trafienie"
			});
		}
		...
		if (this.getContainer().getActor().getSkills().hasSkill("trait.oath_of_fortification"))
		{
			ret.push({
				...
				text = "Ma [color=" + this.Const.UI.Color.PositiveValue + "]100%[/color] na wytrącenie celu z równowagi po trafieniu"
			});
		}

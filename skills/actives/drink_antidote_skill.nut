this.drink_antidote_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Wypij lub Podaj Antidotum";
		this.m.Description = "Podaj pobliskiemu sojusznikowi lub sam wypij to antidotum, które natychmiastowo zneutralizuje wszelkie trucizny i jady. Nie można użyć, gdy wróg zbliży się na odległość walki wręcz, a osoba otrzymująca przedmiot musi mieć wolne miejsce w swojej podręcznej sakwie.";
	...
	function getTooltip()
	{
		local ret = [
			...
			},
			{
				...
				text = "Usuwa zatrucie i jad"
			},
			{
				id = 8,
				type = "text",
				icon = "ui/icons/special.png",
				text = "Przyznaje niewrażliwość na Truciznę na 3 tury"
			}
		];
	...
	function onUse( _user, _targetTile )
	{
		...
		if (_user.getID() == user.getID())
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(user) + " wypija Antidotum");
			}
		...
		else
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " podaje Antidotum do: " + this.Const.UI.getColorizedEntityName(user));
			}

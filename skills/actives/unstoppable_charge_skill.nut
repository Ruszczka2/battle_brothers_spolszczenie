this.unstoppable_charge_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Niepowstrzymana Szarża";
		this.m.Description = "";
	...
	function applyEffectToTarget( _user, _target, _targetTile )
	{
		...
		else if (applyEffect == 2 && !_target.getCurrentProperties().IsImmuneToKnockBackAndGrab && !_target.getCurrentProperties().IsRooted)
		{
			...
			if (!_user.isHiddenToPlayer() && _targetTile.IsVisibleForPlayer)
			{
				this.Tactical.EventLog.log(this.stagger.getLogEntryOnAdded(this.Const.UI.getColorizedEntityName(_user), this.Const.UI.getColorizedEntityName(_target)));
			}
	...
	function onTeleportDone( _entity, _tag )
	{
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_entity) + " szarżuje");
		}

this.shieldwall_effect <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Description = "Ta postać zyskała premię do obrony dzięki uniesieniu tarczy i zajęciu defensywnej postawy.";
	...
	function getTooltip()
	{
		...
		return [
			...
			},
			{
				id = 2,
				type = "description",
				text = this.getDescription() + (bonus > 0 ? " Formując solidną ścianę tarcz z pobliskimi sojusznikami, ochrona jest jeszcze bardziej zwiększona." : "")
			},
			{
				id = 10,
				type = "text",
				icon = "ui/icons/melee_defense.png",
				text = "[color=" + this.Const.UI.Color.PositiveValue + "]+" + this.Math.floor(item.getMeleeDefense() * mult + bonus) + "[/color] do Obrony w zwarciu"
			},
			{
				id = 11,
				type = "text",
				icon = "ui/icons/ranged_defense.png",
				text = "[color=" + this.Const.UI.Color.PositiveValue + "]+" + this.Math.floor(item.getRangedDefense() * mult + bonus) + "[/color] do Obrony dystansowej"
			}
		];
	}

this.acid_effect <- this.inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		...
		if (!this.getContainer().getActor().getFlags().has("head_immune_to_acid"))
		{
			ret.push({
				...
				text = "Traci [color=" + this.Const.UI.Color.NegativeValue + "]20%[/color] pancerza głowy na turę"
			});
		}
		...
		if (!this.getContainer().getActor().getFlags().has("body_immune_to_acid"))
		{
			ret.push({
				...
				text = "Traci [color=" + this.Const.UI.Color.NegativeValue + "]20%[/color] pancerza ciała na turę"
			});
		}
	...
	function onAdded()
	{
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " zostaje spryskany kwasem");
		}
	}

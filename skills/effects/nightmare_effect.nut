this.nightmare_effect <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Koszmary";
		this.m.KilledString = "Umarł w czasie snu";
	...
	{
		return "Ta postać pochłonięta jest przez nienaturalne koszmary i nie jest w stanie działać. Podczas gdy przerażające wizje i mary z wolna konsumują jej zdrowy rozsądek, będzie tracić [color=" + this.Const.UI.Color.PositiveValue + "]" + this.getDamage() + "[/color] punktów zdrowia na turę, bazując na jej stanowczości. Postać może zostać siłą wybudzona z tych koszmarów przez pobliskiego sojusznika, lecz sama nie da rady wyrwać się z upiornego snu.";
	}
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-300[/color] do Inicjatywy"
			}
		];
	}

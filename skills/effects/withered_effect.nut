this.withered_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return "Z powodu mrocznej magii ta postać została wyssana z życia i ma żwawość uschniętego starucha. Efekt powoli minie w przeciągu [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.TurnsLeft + "[/color] " + (this.m.TurnsLeft > 1 ? "tur" : "tury") + ".";
	}
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-" + this.m.TurnsLeft * 25 + "%[/color] do zadawanych obrażeń"
			},
			{
				id = 12,
				type = "text",
				icon = "ui/icons/initiative.png",
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-" + this.m.TurnsLeft * 25 + "%[/color] do Inicjatywy"
			},
			{
				id = 12,
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-" + this.m.TurnsLeft * 25 + "%[/color] maksymalnego zmęczenia"
			},
			{
				id = 11,
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-" + this.m.TurnsLeft * 5 + "[/color] odnowy Zmęczenia na turę"
			}
		];
	}
	...
	function onAdded()
	{
		...
		if (statusResisted)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " szybko otrząsa się z uwiądu dzięki swej niezwykłej fizjologii");
			}

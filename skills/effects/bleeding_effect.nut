this.bleeding_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return "Ta postać obficie krwawi z otrzymanej niedawno rany i będzie tracić [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.Damage + "[/color] punktów zdrowia co turę jeszcze przez [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.TurnsLeft + "[/color] " + (this.m.TurnsLeft > 1 ? "tury" : "turę") + ".";
	}
	...
	function onAdded()
	{
		if (this.getContainer().getActor().getCurrentProperties().IsResistantToAnyStatuses && this.Math.rand(1, 100) <= 50)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " szybko przestaje krwawić z rany, dzięki swej niezwykłej fizjologii");
			}

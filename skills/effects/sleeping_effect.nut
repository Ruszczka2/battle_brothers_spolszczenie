this.sleeping_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return "Ta postać zapadła w nienaturalny sen i nie może podejmować żadnych akcji. Obudzi się sama za [color=" + this.Const.UI.Color.NegativeValue + "]" + (this.m.TurnsLeft - 1) + "[/color] " + (this.m.TurnsLeft > 1 ? "tury" : "turę") + ", choć może też zostać siłą przebudzona przez sojusznika, lub gdy otrzyma jakiekolwiek obrażenia.\n\nIm wyższa stanowczość postaci, tym większa szansa na odparcie nagłej potrzeby snu.";
	}
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-100[/color] do Inicjatywy"
			},
			{
				id = 9,
				type = "text",
				icon = "ui/icons/special.png",
				text = "Może otrzymywać obrażenia od Alpów"
			}
		];
	}

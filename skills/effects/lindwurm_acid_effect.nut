this.lindwurm_acid_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return "Pancerz tej postaci został spryskany przez żrącą krew Lindwurma, która będzie go teraz korodować jeszcze przez [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.TurnsLeft + "[/color] " + (this.m.TurnsLeft > 1 ? "tury" : "turę") + ".";
	}
	...
	function getTooltip()
	{
		...
		if (!this.getContainer().getActor().getFlags().has("head_immune_to_acid"))
		{
			ret.push({
				...
				text = "Traci [color=" + this.Const.UI.Color.NegativeValue + "]10%[/color] pancerza głowy na turę"
			});
		}
		...
		if (!this.getContainer().getActor().getFlags().has("body_immune_to_acid"))
		{
			ret.push({
				...
				text = "Traci [color=" + this.Const.UI.Color.NegativeValue + "]10%[/color] pancerza ciała na turę"
			});
		}
	...
	function onAdded()
	{
		if (this.getContainer().getActor().getCurrentProperties().IsResistantToAnyStatuses && this.Math.rand(1, 100) <= 50)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " pozostaje niewzruszony na kwas dzięki swej niezwykłej fizjologii");
			}
		...
		else
		{
			...
			if (!this.getContainer().getActor().isHiddenToPlayer())
			{
				this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " został spryskany kwasem");
			}
		}
	}

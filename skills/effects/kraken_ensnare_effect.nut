this.kraken_ensnare_effect <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Oplątanie";
		this.m.Description = "Ta postać została oplątana przez gigantyczną mackę, przyciągającą ją bliżej do rozdziawionej paszczy, która grozi pożarciem w całości.";
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				id = 2,
				type = "description",
				text = this.m.Mode == 0 ? this.getDescription() : "Ta postać została oplątana przez gigantyczną mackę, przyciągającą ją bliżej do rozdziawionej paszczy, która grozi pożarciem w całości. Każdej tury macka grozi zmiażdżeniem postaci przez zwiększenie nacisku, co zaskutkuje utratą [color=" + this.Const.UI.Color.NegativeValue + "]10[/color] punktów zdrowia."
			},
			{
				id = 9,
				type = "text",
			...
			},
			{
				...
				type = "text",
				icon = "ui/icons/action_points.png",
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Nie może używać umiejętności[/color]"
			}
		];
	}

this.discover_location_contract <- this.inherit("scripts/contracts/contract", {
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			...
			Title = "W trakcie podróży...",
			Text = "[img]gfx/ui/events/event_76.png[/img]Przyjmujesz propozycję mężczyzny, który zgodnie z obietnicą zdrada ci szczegóły.%SPEECH_ON%No wiesz, to tam, oczywiście, %terrain% %distance% na %direction% od miejsca, gdzie teraz jesteśmy. Łatwizna.%SPEECH_OFF%Pogwizduje sobie, odchodząc, bez wątpienia była to dla niego bardzo łatwa forsa.",
			Image = "",
			List = [],
			Options = [
				{
